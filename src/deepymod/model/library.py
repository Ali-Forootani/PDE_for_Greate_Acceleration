...
class Library2D_4O(Library):
    ...
    def library(
        ...
        du3_x = grad(
            u_xx, data, grad_outputs=torch.ones_like(prediction), create_graph=True
        )[0]
        u_xxx = du3_x[:, 1:2]
        u_xxy = du3_x[:, 2:3]
        ...
        du3_y = grad(
            u_yy, data, grad_outputs=torch.ones_like(prediction), create_graph=True
        )[0]
        u_yyx = du3_y[:, 1:2]
        u_yyy = du3_y[:, 2:3]
        ...
        du4_x = grad(
            u_xxx, data, grad_outputs=torch.ones_like(prediction), create_graph=True
        )[0]
        u_xxxx = du4_x[:, 1:2]
        u_xxxy = du4_x[:, 2:3]
        ...
        du4_xy = grad(
            u_xxy, data, grad_outputs=torch.ones_like(prediction), create_graph=True
        )[0]
        u_xxyy = du4_xy[:, 2:3]
        ...
        du4_y = grad(
            u_yyy, data, grad_outputs=torch.ones_like(prediction), create_graph=True
        )[0]
        u_yyyx = du4_y[:, 1:2]
        u_yyyy = du4_y[:, 2:3]
        ...
        du4_yx = grad(
            u_yyx, data, grad_outputs=torch.ones_like(prediction), create_graph=True
        )[0]
        ...
        du_fin = torch.cat((torch.ones_like(u_x), u_x, u_y, u_xx, u_yy, u_xxxx, u_xxyy, u_yyxx, u_yyyy), dim=1)
        ...
        samples = du_fin.shape[0]
        # Bringing it together
        #theta = torch.matmul(u[:, :, None], du_fin[:, None, :]).view(samples, -1)
