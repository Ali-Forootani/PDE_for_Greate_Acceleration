...
from GNSINDy.src.deepymod.data import Dataset
...
def burgers_delta_org(x: torch.tensor, t: torch.tensor, v: float, A: float):
    """Function to load the analytical solutions of Burgers equation with delta peak initial condition: u(x, 0) = A delta(x)
    ...
    deim_instance = DEIM(u, 1, t.reshape(-1, 1), x.reshape(-1, 1), num_basis = 2)
